def nb_repetitions(elt, tab):
  """
  Entrées:
    - elt, element dont on veut compter le nombre d'occurences
  ...
  Précondition: tab doit être une liste
  Postcondition: nb tel que nb = nombre d'occurences de elt dans tab
  """
  ...
  for element in tab: # On parcourt tab
    if element == elt: # Si l'élement est présent on incrément nb
      nb += 1
  return nb
